...
export function DateFilterReactCalendar({ title, onApply, initialFrom, initialTo }) {
    const [open, setOpen] = useState(false);
    ...
    const handleDateSelect = (value) => {
        if (Array.isArray(value)) {
            ...
            setDateRange(prev => {
                ...
                if (prev.from) newFrom.setHours(prev.from.getHours(), prev.from.getMinutes(), 0, 0);
    ...
    // React-Calendar specific CSS overrides
    const calendarStyles = `
        .react-calendar { border: none; font-family: inherit; width: 100%; }
        .react-calendar__tile--active { background: #0f172a !important; color: white !important; border-radius: 0.375rem; }
        .react-calendar__tile--hasActive { background: #0f172a; color: white; }
        .react-calendar__tile:hover { background: #f1f5f9 !important; border-radius: 0.375rem; }
        .react-calendar__tile { padding: 0.75em 0.5em; font-size: 0.875rem; }
        .react-calendar__navigation button:hover { background: #f1f5f9 !important; border-radius: 0.375rem; }
    `;
                ...
                <div className="absolute left-0 top-full z-50 mt-2 w-[calc(100vw-2rem)] max-w-[600px] rounded-xl border border-slate-200 bg-white p-0 shadow-lg sm:w-auto">
                    ...
                    <div className="flex flex-col gap-4 p-4 sm:flex-row">
                        <div className="flex w-full flex-col gap-3 sm:w-[300px]">
                            <Calendar
                                selectRange={true}
                                value={dateRange.from && dateRange.to ? [dateRange.from, dateRange.to] : dateRange.from}
