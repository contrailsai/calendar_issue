...
export function DateFilterPopoverFixed({ title, onApply, initialFrom, initialTo }) {
    ...
    const handleDateSelect = (newRange, selectedDay, activeModifiers) => {
        // Bail out if the day is physically disabled (boundary hit)
        ...
        let rawTo = new Date(selectedDay); // use fresh copy
    ...
    // --- FIX APPLIED HERE ---
    // Memoize the disabled prop using endOfDay to prevent millisecond re-evaluation issues
